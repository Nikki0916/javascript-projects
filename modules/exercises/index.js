...
const printAll = require('readline-sync') //Import function from display.js.
const randomSelect = require('readline-sync')//Import function from randomSelect.js.
...
for (let i = 0; i<prompts.length; i++){
  ...
  if (response.toLowerCase()==='y'){
    if (i===0){
      print.printAll(astronauts,testTitles,scores)
      //Call 'printAll' here and pass in all necessary arguments.
    } else if (i===1){
      for (let j = 0; j<testTitles.length; j++){
        let avg = averages.averageforStudent//Call 'averageForTest' here. Pass in j and scores as arguments.
